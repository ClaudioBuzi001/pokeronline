...
public class TavoloController {
	...
	public TavoloDTO createNew(@Valid @RequestBody TavoloDTO tavoloInput) {
		...
		*/
		...
		tavoloInput.setUtenteCreazione(UtenteDTO.buildUtenteDTOFromModelNoPassword(utenteService.findByUsername(SecurityContextHolder.getContext().getAuthentication().getName())));
	...
	/**
	 * All interno di questo metodo Utilizziamo due findByExample. Uno per l' admin che può ricercare tutti i tavoli.
	 * Uno per lo SpecialPlayer, che può vedere solo i tavoli gestiti da lui.
	 * Con un controllo verifico Se l' utente in sessione è un admin: findByExample con tutti i tavoli
	 * Se l' utente in sessione è uno SpecialPlayer; findByExampleSpecialPlayer Con solo i tavoli gestiti dall' utente
	 * 
	...
	public List<TavoloDTO> search(@RequestBody TavoloDTO example) {
		...
		if(example.getUtenteCreazione() != null)
			throw new UtenteCreazionePresenteException("All interno del tavolo Example è presente un UtenteCreazione, Impossibile procedere, rimuovi l' utenteCreazione dall' example");
		
		return TavoloDTO.createTavoloDTOListFromModelList(tavoloService.findByExampleSpecialPlayer(example.buildTavoloModel(), inSessione.getId()), false);
	}
